...
def menu_dev():
	...
	print(" [*] Expired : Lifetime")
	print(" [*] ---------------------------------------------")
	print(" [*] IP          : %s\n"%(IP))
	print(" [ Welcome %sUser Premium!%s ]\n"%(K,N))
	...
	if pil == "1" or pil == "01":
		...
		print("\n [ Pilih Target Untuk Di crack ]")
		print("\n [1]. Followers %s : %s%s%s"%(username,H,str(pengikut),N))
		print(" [2]. Following %s : %s%s%s"%(username,H,str(mengikuti),N))
		pil2=raw_input("\n [?] pilih : ")
		if pil2 == "1":
